...
customer_order_history as (
    select 
        ...
        coalesce(count(
            case when orders.valid_order_date is not null then 1 end),0
            ) as non_returned_order_count,
        
        sum(case when valid_order_date is not null 
            then orders.order_value_dollars else 0 end) as total_lifetime_value,
        
        sum(case when valid_order_date is not null 
            then orders.order_value_dollars else 0 end) / nullif(
