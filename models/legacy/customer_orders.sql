...
from  {{ source('dbt_migration_tedo', 'orders') }} as orders

join (
      select 
        first_name || ' ' || last_name as name, 
        * 
      from {{ source('dbt_migration_tedo', 'customers') }}
) customers
...
join (
    ...
    from (
      select 
        ...
        *
      from {{ source('dbt_migration_tedo', 'orders') }} 
    ) a
    ...
    left outer join {{ source('dbt_migration_tedo', 'payments') }} c
    on a.id = c.orderid
...
left outer join {{ source('dbt_migration_tedo', 'payments') }} payments
